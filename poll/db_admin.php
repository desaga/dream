...
// ~~~~~~~~~START~~~~~~~~~~~
// STATISTIC
if($do=="stat"){ //если передан запрос 'stat' выведем статистику
if(isset($_GET['id'])){//если выбрано имя запроса name меняем на id 
// ++++++++ Заменить на поиск информации из БД poll -> id,text,variants,date_start,date_finish
	//echo "ID опроса $_GET[id]";
	//$dat=file($pollcfg);
...
// ADD Добавляем варианты ответов на опрос ГОТОВО
if($do=="add" && isset($_GET['num'])){ //если передан запрос 'add'  и количество вопросов, добавим новый опрос
// Проверка заполнения полей формы.
	if($_GET['question']!="" && $_GET['num']!=""){ //вопрос, к-во вариантов 
		if(is_numeric($_GET['num'])){//если количество вопросов является числом,то
				$addp=1;
				$error="Первый шаг успешно пройден!";
		}else{//введено не числовое значение количества вариантов ответа
			$addp=0;
			$error="Задайте правильное значение количества вариантов ответа!";
		}
	}else{//если какое-то поле ввода не заполнено
		$addp=0;
		$error="Все формы должны быть заполнены!";
	}
	if(isset($_GET['a1'])){//если введен 1-й вариант ответа, обрабатываем ответы
		//Check2 проверим, введены ли все варианты ответов
		for($i=0;$i<$_GET['num'];$i++){
			$il=$i+1;
			if($_GET['a'.$il]==""){
				$non=true;//не введен вариант ответа
			}
		}
		if(!$non){//если введены все варианты ответов
			//add
			$_GET['question']=stripslashes($_GET['question']);
			$_GET['question']=ereg_replace("\"","&quot;",$_GET['question']);//заменяем спецсимволы
			$str1=$_GET['name']."|::|";//перемення для записи в файл опроса
			$str2=$_GET['name']."|::|".$_GET['question']."|::|";//перемення для записи в файл данных опроса
			for($i=0;$i<$_GET['num'];$i++){
				$il=$i+1;
				$str1=$str1."0|::|";//заносим ноли в результаты голосования
				$str2=$str2.$_GET['a'.$il]."|::|";//заносим в файл варианты ответов
			}
			$str1=$str1.tdate($datechar)."|::|\n";//добавляем дату к имени опроса и конец строки
			$str2=$str2."\n";//добавляем конец строки к вариантам опроса
...
// DELETE
if($do=="del" && isset($_GET['id'])){//если передан запрос 'del', удаляем опрос
	//$data1=explode("|::|",$GET[$i]);//получим строку из массива опросов
	$query="DELETE FROM `poll` WHERE `id`=$_GET[id]";//выберем все записи из таблицы опросов
	$result = mysql_query($query) or die(mysql_error());
	
	if($result){//имя опроса равно выбраному?
		$ex=true;//нашли опрос
	}
	
	if($ex){// если опрос был найден
		$del=true;
		$error="Опрос № <b>".$_GET['id']." - '".$_GET['text']."' </b> удалён!";
	}else{// опрос не найден
		$del=false;
		$error="Опрос не найден!";
	}
}
// CHANGE PASSWORD
if($do=="cp" && isset($_GET['npass'])){//если передан запрос 'cp', поменяем пароль
	
	$passmd5=md5($_GET['npass']);//зашифруем новый пароль
	$sql = "UPDATE `users` SET `pass` = '$passmd5' WHERE `name` = '000282018' LIMIT 1";
	$result = mysql_query ($sql);
	echo $result;
	if ($result)
	{
		$error="Вы сменили свой пароль на \"".$_GET['npass']."\"";//создаем сообщение
		$_GET['pass']=$passmd5; //зашифрованый пароль поместим в массив GET
		$cpok=true; //маркер пароль изменен
		echo "Данные успешно обновлены.";
	}
	else
	{
		$error="Ошибка запроса к Базе Данных. Пароль не изменен!!!";
	}
...
<?
if ($do!="add" && $do!="del" && $do!="stat" && $do!="cp"){//если не выбраны действия
?>
...
if(!$stat){//если такой опрос не найден - выводим список опросов
	...
	//$dat1=file($polldata);//файл опроса со статистикой
	//$dat2=file($pollcfg);//файл с опросом/вариантами ответов
